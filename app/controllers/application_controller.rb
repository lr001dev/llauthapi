class ApplicationController < ActionController::API
  ...
  def authenticate_token
    render json: { status: 401, message: 'Unauthorized' } unless
    decode_token(cookies.signed[:jwt])
    end
    ...
    def decode_token(token_input)
      ...
      def authorize_user
        ...
        render json: { status: 401, message: "Unauthorized" } unless get_current_user.id ==
        params[:id].to_i
      end
end
